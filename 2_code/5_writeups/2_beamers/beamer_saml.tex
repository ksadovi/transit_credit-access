...
\subsection{Endogenous Station Location Choice}
...
\subsection{Non-Random Exposure}
...
\begin{frame}{\cite{borusyak_nonrandom_2023}: Recentered Instrument}
\textbf{Problem:}
\begin{itemize}
    \item Many treatments are \emph{constructed} using a known formula
    \[
        z_i = f(g; w_i)
    \]
    \item Some inputs are plausibly exogenous shocks ($g$),
          but others are predetermined and non-random ($w_i$)
    \item Units differ systematically in how much they are exposed to the same shocks $\rightarrow$ OVB even under random shock timing
\end{itemize}
\textbf{Solution: Recenter the instrument}
\[
\tilde{z}_i = z_i - \mu_i,
\qquad
\mu_i = \mathbb{E}[z_i \mid w_i]
\]

\begin{itemize}
    \item $\mu_i$ = expected exposure given geography, networks, or baseline structure
    \item $\tilde{z}_i$ = recentered instrument
\end{itemize}

\end{frame}

\begin{frame}{\cite{borusyak_nonrandom_2023}: High-Speed Rail Application}
\textbf{Empirical setting:}
\begin{itemize}
    \item Study effects of China’s high-speed rail (HSR) expansion on local outcomes
    \item Rail lines open at different times, creating plausibly exogenous timing shocks
    \item Cities differ mechanically in how much HSR access they gain
    \item Central or well-connected cities would benefit more \emph{under any timing}
\end{itemize}
\textbf{Exposure construction:}
\[
z_{it} = \sum_{s} \mathbf{1}\{\text{HSR segment } s \text{ open at } t\}
        \times \text{Connectivity}_{is}
\]
\begin{itemize}
    \item Exposure depends on:
    \begin{itemize}
        \item shock realization (which segments open when),
        \item baseline geography and network position.
    \end{itemize}
\end{itemize}
\end{frame}

\begin{frame}{\cite{borusyak_nonrandom_2023}: High-Speed Rail Application}
\textbf{Instrument: recentered exposure}
\[
\tilde{z}_{it} = z_{it} - \mathbb{E}[z_{it} \mid \text{baseline network}]
\]

\begin{itemize}
    \item Expected exposure computed by permuting opening dates across segments
    \item Identification comes from \emph{unexpected} early or late access
    \item Effect of increased HSR access driven purely by quasi-random timing
    \item \textbf{Note:} Anticipation effects remain unacknowledged.
\end{itemize}
\end{frame}
...
\begin{frame}{Example: Isochrones and Worker Flows}
\centering
\begin{subfigure}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{3_output/2_figures/1_maps/1_station_geographies/WMATA.pdf}
...
\small
Following \citeauthor{eaton_technology_2002}, \citeauthor{donaldson_railroads_2016}, and \citeauthor{tsivanidis_evaluating_2022}'s gravity equation logic:
\[
